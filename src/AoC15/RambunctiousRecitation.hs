...
recite2020 ints goal = lastEntered $ reciteRecursive $ PS toIntMap (length ints + 1) (last ints) (reverse ints) goal
...
reciteRecursive params@(PS indexMap' count' lastEntered' ll goal')
    ...
    where indexes = IM.findWithDefault [] lastEntered' indexMap'
          ...
          valueToInsert = let existing = IM.findWithDefault [] numberToInsert indexMap' in count':existing
